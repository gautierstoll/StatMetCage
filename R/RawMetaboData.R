...
#' If no fileName submit create an empty RawMetaData object
...
setMethod(f="initialize",
          ...
          definition = function(.Object,fileName,sepCSV = ";"){
            if (missingArg(fileName)){
              ...
              Data <- scan(fileName,what="",sep="\n",dec=",",blank.lines.skip = FALSE)
              Data <- Data[Data != ""]
              tmpFirst <- grep("Date;Time", Data, useBytes = TRUE) # Find the empty row as separation
              # Meta info
              ...
              .Object@data = DataDF %>% as_tibble %>% mutate(OriginDate = .Object@Date)
              .Object@header <- .Object@header %>% as_tibble %>% mutate_at(3, function(x) as.numeric(gsub(",",".", x))) %>% mutate(Date = DataDF[1,1], Time = DataDF[1,2])
            }
            return(.Object)
          })
